...
class SelfAttention(tf.keras.Model):
    ...
    def call(self, input_):
        ...
        if self.sa_type == 0:
            ...
            padding = tf.constant([[0, 0], [self.kernel_size//2, self.kernel_size//2], [self.kernel_size//2, self.kernel_size//2], [0, 0]])

            if self.strides !=1:
                x1 = tf.image.extract_patches(x1, kernel_size_unfold_i, strides_unfold, dilation_unfold, "VALID")
            x1 = tf.reshape(x1, [tf.shape(input_)[0], input_.shape[1], 1, input_.shape[2]*input_.shape[3]])
            
            x2 = tf.reshape(tf.image.extract_patches(tf.pad(x2, padding, "REFLECT"), 
                            kernel_size_unfold_j, strides_unfold, dilation_unfold, "SAME"),
                            [tf.shape(input_)[0], input_.shape[1], 1, x1.shape[-1]])
            ...
            w = tf.reshape(out, [tf.shape(input_)[0], input_.shape[1], pow(self.kernel_size, 2), x1.shape[-1]])
...
#         self.conv1 = conv_1x1(rel_planes)
#         self.conv2 = conv_1x1(rel_planes)
#         self.conv3 = conv_1x1(rel_planes)
#         if sa_type == 0:
#             self.conv_w = tf.keras.Sequential([tf.keras.layers.BatchNormalization(),
#                                             tf.keras.layers.ReLU(),
#                                             conv_1x1(rel_planes),
...
#             if self.strides !=1:
#                 x1 = tf.image.extract_patches(x1, kernel_size_unfold_i, strides_unfold, dilation_unfold, "VALID")
#             x1 = tf.reshape(x1, [input_.shape[0], -1, 1, input_.shape[2]*input_.shape[3]])
            
#             x2 = tf.reshape(tf.image.extract_patches(tf.pad(x2, padding, "REFLECT"), 
#                             kernel_size_unfold_j, strides_unfold, dilation_unfold, "SAME"),
#                             [input_.shape[0], -1, 1, x1.shape[-1]])

#             out = self.conv_w1(self.relu_w1(self.bn_w1(tf.concat([x1, x2], 1))))
#             out = self.conv_w2(self.relu_w2(self.bn_w2(out)))
#             w = tf.reshape(out, [input_.shape[0], -1, pow(self.kernel_size, 2), x1.shape[-1]])
#             print("w", w.shape)
#         x = self.aggregation(x3, w)
